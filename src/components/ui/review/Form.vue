...
<template>
  <div class=" mx-auto">
    ...
    <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" class="space-y-6" label-position="top"
      require-asterisk-position="right">
      <!-- Rating Section -->
      <div class="space-y-2">
        <el-form-item :label="$t('general.rating')" label-position="left" required class="flex space-x-1">
          <el-rate v-model="rating" />
          <div v-if="ratingError" class="el-form-item__error">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <el-form-item :label="$t('general.name')" prop="author">
          <el-input v-model="ruleForm.author" :placeholder="$t('general.name')" type="text" />
        </el-form-item>

        <el-form-item :label="$t('general.email')" prop="email">
          <el-input v-model="ruleForm.email" :placeholder="$t('general.email')" type="email" />
        </el-form-item>
      </div>

      <!-- Comment Textarea -->
      <el-form-item :label="$t('general.comment')" prop="comment">
        <el-input v-model="ruleForm.comment" :placeholder="$t('general.comments')" :rows="6" type="textarea" />
      </el-form-item>

      <!-- Submit Button -->
      <button class="bg-black text-white px-6 py-2 rounded-lg hover:bg-gray-800 transition-colors" type="button"
