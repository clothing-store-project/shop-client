...
<template>
  <div v-if="isOpen" class="min-h-screen h-full bg-white fixed inset-0 z-50">
    <div class="flex items-center justify-between mt-4 px-4">
      <h2 class="text-lg font-medium mx-auto">{{ isNew ? 'Thêm địa chỉ' : 'Sửa địa chỉ' }}</h2>
      <button
    ...
    <el-form ref="ruleFormRef" :model="dialogAddress" :rules="rules" class="mb-4 px-4">
      ...
      <div class="fixed bottom-0 flex mb-4 w-full pr-8">
        <el-button class="w-1/2" @click="$emit('close-popup')">{{ $t('general.cancel') }}</el-button>
        <el-button class="w-1/2" v-if="isNew" type="primary" @click="$emit('confirm', dialogAddress, true)">
          {{ $t('general.add') }}
        </el-button>
        <el-button v-else class="w-1/2" type="primary" @click="$emit('confirm', dialogAddress, false)">
          {{ $t('general.update') }}
        </el-button>
      </div>
    </el-form>
